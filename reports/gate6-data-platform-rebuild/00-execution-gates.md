...
DB·API·backfill·rollback·frontend·release 명세를 격리 브랜치에 확정했습니다. 2026-08-04 계약 감사에서 확인된 자산 공개키, 사용자 JWT RPC, 단일 writer, 월 원장 자연키, 계산 실행 주체, 만기 범위, 이메일 실행 경로를 문서에 잠갔습니다. 원천 승인이 필요한 formula는 승인 전 금액을 만들지 않고 해당 계산·cutover만 차단합니다. 기능 구현은 실패 테스트를 먼저 작성하고, 운영 적용은 additive schema migration의 사전 검증 뒤에만 수행합니다.
...
2026-08-04에 DB·API·계산·frontend 정적 계약 테스트를 먼저 작성하고 실행했습니다. 네 테스트는 모두 아직 구현되지 않은 계약 때문에 의도대로 실패했습니다. 주요 실패는 수익권 tranche·만기 자산 범위·월 원장 원천키·feature flag·분리 grant·승인 formula metadata·finance write 차단의 부재입니다. 이 결과는 구현 완료가 아니라 RED 기준점입니다. 실제 PostgreSQL 반복 migration·backfill·reverse 실행 테스트와 브라우저 E2E 실패 테스트는 아직 남아 있으므로 G5를 `PASS`로 올리지 않습니다.

## G6~G10. 구현·배포 — 차단

R0와 R1 통과 전에는 SDD 문서와 의도적으로 실패하는 테스트 외의 구현을 진척으로 인정하지 않습니다. 이미 존재하는 schema·Edge·frontend 초안은 격리 보존하고 R1 승인 전 더 진행하거나 완료로 계산하지 않습니다. R0·R1 통과 뒤에는 공통 골격 구현을 시작할 수 있지만 finance·메일·약정 계산처럼 외부 운영 원천이 필요한 기능은 각 원천 승인을 받기 전까지 비활성 상태를 유지합니다. R1과 로컬 migration·backfill·reverse 리허설 통과 뒤에는 외부 grant·writer·worker가 모두 비활성인 additive DDL만 같은 프로젝트에 1회 적용하여 R2 production shadow를 검증할 수 있습니다. 공개 권한·쓰기·worker·Edge·frontend 활성화는 R3와 릴리스 게이트 통과 뒤에만 수행합니다.
