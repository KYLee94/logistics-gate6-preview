...
async function main() {
  ...
  const outJson = path.join(OUT_DIR, `news-edge-restore-20260617-${stamp}.json`);
  const latestJson = path.join(OUT_DIR, 'news-edge-restore-20260617-latest.json');
  ...
  const response = await fetch(`${supabaseUrl.replace(/\/$/u, '')}/functions/v1/ll-dashboard-api`, {
    ...
    body: JSON.stringify({ action: 'news/restore-20260617', payload: {} }),
  });
  const body = await response.json().catch(() => ({}));
  const report = {
    ok: response.ok && body?.ok === true && Number(body?.data?.after_count || 0) >= 8 && Number(body?.data?.after_count || 0) <= 10,
  ...
  console.log(JSON.stringify({ ok: report.ok, http_status: response.status, artifact: outJson, after_count: body?.data?.after_count ?? null }, null, 2));
