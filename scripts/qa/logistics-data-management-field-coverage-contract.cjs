...
const requiredChecks = [
  ...
  ['investment_integrated_fields', ['equity_amount_krw', 'loan_amount_krw', 'equity_parties', 'loan_lenders', 'tranche_summary', 'weighted_loan_rate', 'nearest_maturity_date'].every((field) => investmentIntegratedFieldBlock.includes(`field_key: '${field}'`))],
  ...
  ['investment_tranche_summary_has_rate_and_maturity', /trancheSummary/u.test(investmentIntegratedRowsBlock) && /dataManagementFormatViewValue\(rate\(row\),\s*\{\s*type:\s*'percent'\s*\}\)/u.test(investmentIntegratedRowsBlock) && /safeDateText\(row\.maturity_date\)/u.test(investmentIntegratedRowsBlock)],
