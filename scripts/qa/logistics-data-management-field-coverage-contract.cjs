...
const requiredChecks = [
  ...
  ['investment_integrated_fields', ['equity_amount_krw', 'loan_amount_krw', 'total_capital_krw'].every((field) => investmentIntegratedFieldBlock.includes(`field_key: '${field}'`)) && !['equity_parties', 'loan_lenders', 'tranche_summary', 'weighted_loan_rate', 'nearest_maturity_date', 'maturity_summary', 'asset_manager_name'].some((field) => investmentIntegratedFieldBlock.includes(`field_key: '${field}'`))],
  ...
  ['investment_tranche_detail_has_rate_and_maturity', /cell_details:\s*\{/u.test(investmentIntegratedRowsBlock) && /equity_amount_krw:\s*beneficiaryDetail/u.test(investmentIntegratedRowsBlock) && /loan_amount_krw:\s*loanDetail/u.test(investmentIntegratedRowsBlock) && /field_key:\s*'loan_rate'/u.test(investmentIntegratedRowsBlock) && /field_key:\s*'all_in_rate'/u.test(investmentIntegratedRowsBlock) && /safeDateText\(trancheRow\.maturity_date\)/u.test(investmentIntegratedRowsBlock)],
