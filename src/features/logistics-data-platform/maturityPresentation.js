...
export function maturityDetailRows(row = {}) {
  ...
  if (type === 'lease') {
    return [
      ...
      ['보증금(원)', row.deposit_amount, 'amount'],
      ['월 임대료(원)', row.monthly_rent_total_krw, 'amount'],
      ['월 관리비(원)', row.monthly_cam_total_krw, 'amount'],
  ...
  return [
    ...
    ['약정액(원)', row.commitment_amount, 'amount'],
    ['대출잔액(원)', row.outstanding_amount, 'amount'],
    ['대출 유형', row.loan_type],
