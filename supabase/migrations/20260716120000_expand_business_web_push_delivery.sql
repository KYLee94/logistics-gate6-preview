...
declare
  ...
  gateway_jwt text;
begin
  if new.notification_type not in ('task_share', 'data_update', 'lease_maturity', 'loan_maturity', 'system')
    or new.delivery_status = 'dismissed' then
    return new;
  end if;
  ...
  exception
    when others then
      raise warning 'Web push queue request failed for notification %: %', new.notification_id, sqlerrm;
  end;
